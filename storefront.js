import {MEASURE_LABELS,colourProfile,displayColor,displayOrigin,displaySize,displayTitle,publicPairNotes,silhouetteProfile} from './storefront-utils.js';
...
const EVENT_URL='https://qgjkxtolyhbwpvncwtkn.supabase.co/rest/v1/rpc/track_fadewell_storefront_event';
const EUR_CACHE_KEY='fadewell-eur-rate-v1';
const INTERNAL_KEY='fadewell-internal-traffic';
let products=[];
let currency='PLN',eurRate=null;

function internalTraffic(){return localStorage.getItem(INTERNAL_KEY)==='1'}
function applyTrackingPreference(){
  const preference=new URLSearchParams(location.search).get('tracking');
  if(preference==='off')localStorage.setItem(INTERNAL_KEY,'1');
  if(preference==='on')localStorage.removeItem(INTERNAL_KEY);
  if(!internalTraffic())return;
  document.body.insertAdjacentHTML('afterbegin','<div class="internal-traffic" role="status">Internal traffic · analytics paused · <a href="?tracking=on">resume</a></div>');
}
applyTrackingPreference();

function header(){return `<a class="skip-link" href="#main-content">Skip to content</a><div class="site-header"><nav class="nav shell" aria-label="Main navigation"><a class="wordmark" href="/" aria-label="FADEWELL home"><img src="/fadewell-wordmark.png" alt="FADEWELL"></a><button class="mobile-toggle" aria-controls="site-navigation" aria-expanded="false">Menu</button><div class="nav-links" id="site-navigation"><a data-nav="shop" href="/shop.html">Shop</a><a data-nav="finder" href="/finder.html">Denim Finder</a><a data-nav="archive" href="/archive.html">Pair Archive</a><a data-nav="guides" href="/guides.html">Denim Guides</a><a href="https://www.instagram.com/byfadewell/" target="_blank" rel="noopener">Journal ↗</a><button class="currency-toggle" type="button" data-currency-toggle aria-pressed="false" aria-label="Show prices in euro"><span>PLN</span><span aria-hidden="true">/</span><span>EUR</span></button><a class="button primary nav-vinted" href="${VINTED_PROFILE}" target="_blank" rel="noopener">Vinted ↗</a></div></nav></div>`}
...
document.querySelector('.mobile-toggle')?.addEventListener('click',event=>{const nav=document.querySelector('.nav-links');nav.classList.toggle('open');event.currentTarget.setAttribute('aria-expanded',String(nav.classList.contains('open')));event.currentTarget.textContent=nav.classList.contains('open')?'Close':'Menu'});
...
document.querySelector('[data-currency-toggle]')?.addEventListener('click',async event=>{const button=event.currentTarget;button.disabled=true;try{if(currency==='PLN'){await getEurRate();currency='EUR'}else currency='PLN';updatePrices();syncCurrencyButton();trackEvent('currency_toggle')}catch(error){button.title='The NBP EUR rate is temporarily unavailable';trackEvent('storefront_error');console.error(error)}finally{button.disabled=false}});
syncCurrencyButton();

function trackEvent(eventType,vintedItemId=null){
  if(internalTraffic())return;
  const payload={p_event_type:eventType,p_source_page:page,p_vinted_item_id:vintedItemId?String(vintedItemId):null};
  fetch(EVENT_URL,{method:'POST',headers:{apikey:API_KEY,Authorization:`Bearer ${API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify(payload),keepalive:true}).catch(()=>{});
}
if(page&&page!=='pair')trackEvent('page_view');
if(page==='archive')trackEvent('archive_view');
if(page){setTimeout(()=>trackEvent('engaged_view'),30000);let scrollTracked=false;addEventListener('scroll',()=>{if(scrollTracked)return;const max=document.documentElement.scrollHeight-innerHeight;if(max>0&&scrollY/max>=.75){scrollTracked=true;trackEvent('scroll_75')}},{passive:true})}

function safe(value){return String(value??'').replace(/[&<>'"]/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[char]))}
function m(product,key){return product.measurements?.[key]?.cm}
function measurementDisplay(product,key){const value=product.measurements?.[key];if(!value)return null;return value.display||(value.cm!=null?`${value.cm} cm`:null)}
function pairUrl(product){return `/pairs/${encodeURIComponent(product.vinted_item_id)}/`}
function productPhotos(product){return product.display_photos?.length?product.display_photos:(product.photos||[])}
...
function card(product,deltas,options={}){
  const photo=product.card_photo_url||productPhotos(product)[0]||'',showPrice=options.showPrice!==false;
...
function galleryMarkup(product){
  const photos=productPhotos(product),title=displayTitle(product);if(!photos.length)return '<div class="pair-photo-empty">No photo available</div>';
...
function setupGallery(root,product){
  const gallery=root.querySelector('[data-gallery]'),photos=productPhotos(product);if(!gallery||photos.length<2)return;
...
async function renderPair(){
  ...
  try{
    ...
    const measures=Object.entries(MEASURE_LABELS).filter(([key])=>m(product,key)!=null).map(([key,label])=>`<div class="measure-row"><span>${label}</span><strong>${safe(measurementDisplay(product,key))}</strong></div>`).join('');
...
async function setupFinder(){
  ...
  form.addEventListener('input',()=>{if(!finderStarted){finderStarted=true;trackEvent('finder_start')}});
  form.addEventListener('submit',event=>{
    event.preventDefault();trackEvent('finder_submit');
    ...
    if(!entries.length&&!colour&&!silhouette){errorNode.hidden=false;trackEvent('finder_empty');return}
    ...
    const section=document.querySelector('[data-finder-results]'),node=document.querySelector('[data-finder-products]');section.hidden=false;node.dataset.finderResults='true';node.innerHTML=ranked.length?ranked.map(result=>card(result.product,result.deltas)).join(''):'<p class="empty">No available pair has every selected measurement yet.</p>';if(!ranked.length)trackEvent('finder_empty');updatePrices();section.scrollIntoView({behavior:'smooth'});
  });
}

document.addEventListener('click',event=>{
  const pairLink=event.target.closest('[data-pair-link]');
  if(pairLink){trackEvent('pair_card_click',pairLink.dataset.pairId);if(pairLink.closest('[data-finder-results]'))trackEvent('finder_result_click',pairLink.dataset.pairId)}
  const vintedLink=event.target.closest('[data-vinted-link]');
  if(vintedLink)trackEvent('vinted_click',vintedLink.dataset.pairId);
  const localLink=event.target.closest('a[href]');
  if(localLink?.href.includes('/shop.html'))trackEvent('browse_shop');
  if(localLink?.href.includes('/finder.html'))trackEvent('open_finder');
});
