...
function card(product,deltas,options={}){
  ...
  return `<article class="product-card"><a class="card-photo" href="${pairUrl(product)}"><img loading="lazy" src="${safe(photo)}" alt="${safe(displayTitle(product))}"><span class="badge ${product.sold?'sold':''}">${product.sold?'Sold archive':safe(product.garment_type==='JEANS'?'Jeans':'Trousers')}</span></a><div class="card-body"><div class="card-meta"><span>${safe(product.brand||'Unbranded')}</span><span>${safe(displaySize(product))}</span></div><h3 class="card-title"><a href="${pairUrl(product)}">${safe(displayTitle(product))}</a></h3><div class="card-measures"><span>W ${m(product,'waist')??'—'} cm</span><span>R ${m(product,'rise')??'—'} cm</span><span>L ${m(product,'overall_length')??'—'} cm</span></div>${deltas?`<ul class="delta-list">${deltas.map(delta=>`<li>${safe(MEASURE_LABELS[delta.key])}: <strong>${delta.delta>0?'+':''}${delta.delta.toFixed(1)} cm</strong></li>`).join('')}</ul>`:''}${showPrice?`<p class="card-price"${product.sold?'':` data-price-pln="${safe(product.price_pln)}"`}>${product.sold?'Reference pair':money(product.price_pln)}</p>`:''}</div></article>`;
}
...
function galleryMarkup(product){
  ...
  return `<div class="pair-gallery" data-gallery><div class="pair-hero-photo"><img loading="eager" data-gallery-hero src="${safe(photos[0])}" alt="${safe(title)}, selected photo"></div>${photos.length>1?`<div class="pair-gallery-rail"><button class="gallery-arrow" type="button" data-gallery-prev aria-label="Previous photos">←</button><div class="pair-thumbs" data-gallery-thumbs></div><button class="gallery-arrow" type="button" data-gallery-next aria-label="Next photos">→</button></div>`:''}</div>`;
}
function setupGallery(root,product){
  ...
  const hero=gallery.querySelector('[data-gallery-hero]'),thumbs=gallery.querySelector('[data-gallery-thumbs]'),prev=gallery.querySelector('[data-gallery-prev]'),next=gallery.querySelector('[data-gallery-next]');let start=1,selected=0;
  ...
  thumbs.addEventListener('click',event=>{const button=event.target.closest('[data-gallery-index]');if(!button)return;selected=Number(button.dataset.galleryIndex);hero.src=photos[selected];hero.alt=`${displayTitle(product)}, photo ${selected+1}`;draw()});
